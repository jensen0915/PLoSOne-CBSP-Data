...
grep dumpAllSubflowMPTCP_sentSeq_dsn__sending_queue_size 3 > n_dumpsentSeq_dsn__sending_queue_size_all_subflows.txt 
...
grep rebuffer 3 > playoutLog.txt
grep TrafficTracePacketSendingTime 3 > CBSP_server_packetSendingTime.txt
grep TCP_idle_restart 3 > CWND_RESET_log.txt
grep dumpCWNDResetTime 3 > CWND_RESET_times_log.txt
grep measurementBW 3 > measurementBW.txt
grep dumpSubflowIdle 3 > Subflow_AvgIdleTime.txt
grep dumpUtilization 3 > Utilization.txt
grep opencwnd 3 > opencwnd_increment.txt
grep calculate_alpha 3 > calculate_alpha.txt
